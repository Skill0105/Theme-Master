        ...
        <div class="col-md-2">
            <?php if($isConnected): ?>
                <?php if(!$isLock OR $perms['FORUM_TOPIC_LOCK']): ?>
                    <a href="<?= $this->Html->url('/topic/add/'.$id) ?>" style="height: 42px;padding-top: 11px;" class="btn btn-primary mt20"><i class="fa fa-plus" aria-hidden="true"></i> <?= $Lang->get('FORUM__TOPIC__CREATE'); ?></a>
                <?php endif; ?>
            <?php endif; ?>
        </div>
    </div>
