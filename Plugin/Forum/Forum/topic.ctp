...
<div class="background-forum">
                ...
                <div class="col-md-2 col-xs-12 text-center">
                    <?php if ($perms['FORUM_TOPIC_LOCK']): ?>
                        <form style="width: 46px" class="inline" action="" method="post">
                            ...
                            <button type="submit" data-toggle="tooltip" data-placement="bottom" name="lock"
                                    value="<?= $id; ?>"
                                    title="<?php if ($lock): ?><?= $Lang->get('FORUM__UNLOCK'); ?><?php else: ?><?= $Lang->get('FORUM__LOCK'); ?><?php endif; ?>"
                                    style="height: 42px;padding-top: 11px;"
                                    class="btn btn-primary mt20"><?php if (!$lock): ?><i class="fa fa-lock"
                                                                                         aria-hidden="true"></i><?php else: ?>
                                    <i class="fa fa-unlock" aria-hidden="true"></i><?php endif; ?></button>
                        </form>
                    <?php endif; ?>
                    <?php if ($perms['FORUM_TOPIC_STICK']): ?>
                        <form style="width: 46px" class="inline" action="" method="post">
                            ...
                            <button type="submit" data-toggle="tooltip" data-placement="bottom"
                                    title="<?php if ($stick): ?><?= $Lang->get('FORUM__UNSTICK'); ?><?php else: ?><?= $Lang->get('FORUM__STICK'); ?><?php endif; ?>"
                                    name="stick" value="<?= $id; ?>" style="height: 42px;padding-top: 11px;"
                                    class="btn btn-primary mt20"><i class="fa fa-paperclip" aria-hidden="true"></i>
                            </button>
                        </form>
                    <?php endif; ?>
                    <?php if (($perms['FORUM_TOPICMY_DELETE'] && $_SESSION['user'] == $msgs[0]['Topic']['id_user']) || $perms['FORUM_TOPIC_DELETE']): ?>
                        <form style="width: 46px" class="inline" action="" method="post">
                            <input name="data[_Token][key]" value="<?= $csrfToken ?>" type="hidden">
                            <button type="submit" data-toggle="tooltip" data-placement="bottom" title="Supprimer"
                                    name="deleteall" value="<?= $id; ?>" style="height: 42px;padding-top: 11px;"
                                    class="btn btn-primary mt20"><i class="fa fa-times" aria-hidden="true"></i></button>
                        </form>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
                            ...
                            <div class="col-md-3">
                                <div class="forum-topic-left">
                                    <div class="forum-forum-withoutmarge">
                                        <img class="center-block topic-avatar"
                                             src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin', 'plugin' => false, $msg['Topic']['author'], '120', '42')); ?>"
                                             alt="Avatar <?= $msg['Topic']['author']; ?>"/>
                                        <p class="text-center"><strong><a
                                                        style="color:#<?= $msg['Topic']['author_color']; ?>"
                                                        href="<?= $this->Html->url('/user/' . $msg['Topic']['author'] . '.' . $msg['Topic']['id_user'] . '/'); ?>"><?= $msg['Topic']['author']; ?></a></strong>
                                        </p>
                                        <div class="forum-rank">
                                            <?php if (!empty($msg['Topic']['author_info']['rank'])): ?>
                                                <?php foreach ($msg['Topic']['author_info']['rank'] as $key => $rank): ?>
                                                    <div style="background-color:#<?= $msg['Topic']['author_info']['color'][$key]['color']; ?>"
                                                         class="forum-badgerank forum-topic-badgerank"><?= $rank['group_name']; ?></div>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                        </div>
                                        <div class="forum-extrainfo">
                                            <dl>
                                                <dt><?= $Lang->get('FORUM__MSG'); ?><?php if ($msg['Topic']['author_info']['nb_message'] > 1) echo 's'; ?>
                                                    :
                                                </dt>
                                    ...
                                    <div class="topic-footer-info">
                                        ...
                                        <div class="topic-footer-rate">
                                            <?php if (isset($_SESSION['user']) && $_SESSION['user'] == $msg['Topic']['id_user'] && $perms['FORUM_MSGMY_EDIT'] || $perms['FORUM_MSG_EDIT']): ?>
                                                <a href="#" onclick="getMessage(<?= $msg['Topic']['id']; ?>)"
                                            ...
                                            <?php if (isset($_SESSION['user']) && $_SESSION['user'] == $msg['Topic']['id_user'] && $perms['FORUM_MSGMY_DELETE'] || $perms['FORUM_MSG_DELETE']): ?>
                                                <form action="" method="post">
                                                    ...
                                                    <button type="submit" name="delete"
                                                            value="<?= $msg['Topic']['id']; ?>"
                                                            class="btn btn-primary"><?= $Lang->get('GLOBAL__DELETE'); ?></button>
                                                </form>
                                            <?php endif; ?>
                                            <?php if (isset($_SESSION['user']) && $_SESSION['user'] != $msg['Topic']['id_user'] AND isset($_SESSION['User'])): ?>
                                                <?php if ($perms['FORUM_MSG_REPORT']): ?>
                                                    <a onclick="report(<?= $msg['Topic']['id']; ?>)" data-toggle="modal"
                                                       data-target="#ModalReport"
                                                       class="btn btn-primary"><?= $Lang->get('FORUM__REPORT'); ?></a>
                                                <?php endif; ?>
                                                <?php if ($active['notemsg']): ?>
                                                    <a id="<?= $msg['Topic']['id']; ?>-1"
                                                       onclick="forumThumb(1, <?= $msg['Topic']['id']; ?>, <?= $msg['Topic']['id_user']; ?>)"
                                                       data-toggle="tooltip" data-placement="top"
                                                       title="<?= $msg['Topic']['thumb']['green']; ?>"
                                                       class="btn-theme thumb-green <?php if ($msg['Topic']['thumb_info']['green'] > 0) echo 'active'; ?>"
                                                       name="voteup" value=""><i class="fa fa-thumbs-up"
                                                                                 aria-hidden="true"></i></a>
                                                    <a id="<?= $msg['Topic']['id']; ?>-2"
                                                       onclick="forumThumb(2, <?= $msg['Topic']['id']; ?>, <?= $msg['Topic']['id_user']; ?>)"
                                                       data-toggle="tooltip" data-placement="top"
                                                       title="<?= $msg['Topic']['thumb']['red']; ?>"
                                                       class="btn-theme thumb-red <?php if ($msg['Topic']['thumb_info']['red'] > 0) echo 'active'; ?>"
                                                       name="votedown" value=""><i class="fa fa-thumbs-down"
                                                                                   aria-hidden="true"></i></a>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                        </div>
                ...
                <form method="post" action="<?= $this->Html->url('/' . $this->request->url); ?>">
                    ...
                    <div class="form-group text-center">
                        <button type="submit" class="btn btn-primary"><i class="fa fa-paper-plane"
                                                                         aria-hidden="true"></i> <?= $Lang->get('FORUM__SEND__MYMSG'); ?>
                        </button>
                    </div>
                </form>
            <?php endif; ?>
    ...
    <?php if (isset($_SESSION['user'])): ?>
        ...
        <!-- Edit -->
        <div class="modal fade bs-example-modal-lg" id="ModalEdit" tabindex="-1" role="dialog"
                ...
                <div class="modal-content">
                    ...
                    <form method="post" action="<?= $this->Html->url('/' . $this->request->url); ?>">
                        ...
                        <div class="modal-footer">
                            <a id="urlEdit" class="btn btn-info pull-left" href=""><i class="fa fa-pencil"
                                                                                      aria-hidden="true"></i> <?= $Lang->get('FORUM__ADVANCED__EDITOR'); ?>
                            </a>
                            <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-times"
                                                                                                  aria-hidden="true"></i> <?= $Lang->get('GLOBAL__CLOSE'); ?>
                            </button>
                            <button type="submit" id="submit_update" class="btn btn-primary"><i class="fa fa-pencil"
                                                                                                aria-hidden="true"></i> <?= $Lang->get('FORUM__EDIT__MYMSG'); ?>
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <?php if ($perms['FORUM_MSG_REPORT'] && $active['reportmsg']): ?>
        <!-- Report -->
        <div class="modal fade bs-example-modal-lg" id="ModalReport" tabindex="-1" role="dialog"
             aria-labelledby="myModalLabel">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form method="post" action="<?= $this->Html->url('/' . $this->request->url); ?>">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" id="submit_report" class="btn btn-primary"><i
                                        class="fa fa-paper-plane"
                                        aria-hidden="true"></i> <?= $Lang->get('FORUM__SEND__REPORT'); ?></button>
                        </div>
                    </form>
                </div>
            </div>
